...
const EditResume = () => {
  ...
  //         API_PATHS.RESUMES.GET_BY_ID(resumeId)
  //       );
  //       setResumeData(response.data); // ✅ sets correct title and fields
  //     } catch (error) {
 ...
 const updateResumeDetails = async (thumbnailLink = "") => {
  try {
    ...
    // Sanitize resumeData to avoid circular refs
    const {
      title,
      template,
      profileInfo,
    ...
    await axiosInstance.put(API_PATHS.RESUMES.UPDATE(resumeId), payload);
  } catch (err) {
...
const uploadResumeImages = async () => {
  ...
  const handleDeleteResume = async () => {
    try {
      ...
      // Should return something like "/api/resumes/123"
